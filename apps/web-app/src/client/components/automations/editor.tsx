...
const AUTOMATION_STEP_TYPES: FlowStep["type"][] = [
  "text",
  "audio",
  "image",
  "video",
    ...
    <div className="space-y-5">
      ...
      {isInstagramIncoming && (
        <div className="rounded-2xl border border-emerald-400/20 bg-emerald-400/[0.08] px-4 py-3 text-sm text-emerald-100">
          Dispara quando entra uma nova mensagem no Instagram e responde pelo thread ja
          sincronizado.
        </div>
      )}
        ...
        <label className="space-y-2 text-sm">
          ...
          <Input
            ...
            onChange={(event) =>
              onChange({
                ...value,
                triggerTags: event.target.value
                  .split(",")
                  .map((item) => item.trim())
                  .filter(Boolean),
              })
        ...
        <label className="space-y-2 text-sm">
          ...
          <Input
            ...
            onChange={(event) =>
              onChange({
                ...value,
                excludeTags: event.target.value
                  .split(",")
                  .map((item) => item.trim())
                  .filter(Boolean),
              })
      ...
      <div className="grid gap-4 lg:grid-cols-4">
        {!isInstagramIncoming ? (
          <label className="space-y-2 text-sm">
            ...
            <Input
              type="number"
              value={value.timeWindowHours}
              onChange={(event) =>
                onChange({ ...value, timeWindowHours: Number(event.target.value) })
              }
            />
          </label>
        ) : (
          <label className="space-y-2 text-sm">
            ...
            <Input
              ...
              onChange={(event) =>
                onChange({ ...value, randomDelayMinSeconds: Number(event.target.value) })
              }
            />
          </label>
        )}
        <label className="space-y-2 text-sm">
          <span className="text-slate-300">
            {isInstagramIncoming ? "Delay max (s)" : "Intervalo minimo (h)"}
          </span>
          <Input
            type="number"
        ...
        <label className="space-y-2 text-sm">
          ...
          <Input
            ...
            onChange={(event) => onChange({ ...value, sendWindowStart: event.target.value })}
          />
        </label>
        <label className="space-y-2 text-sm">
          ...
          <Input
            ...
            onChange={(event) => onChange({ ...value, sendWindowEnd: event.target.value })}
          />
        </label>
      </div>
          ...
          >
            ...
            <Switch
              ...
              onCheckedChange={(checked) =>
                onChange({ ...value, [item.key]: checked } as AutomationDraft)
              }
            />
          </div>
        ))}
