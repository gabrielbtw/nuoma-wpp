    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {value.filters.length > 1 && (
            <button
              onClick={toggleLogic}
              className={cn(
                "flex items-center gap-1 rounded-lg px-2 py-0.5 text-[9px] font-black uppercase tracking-widest border transition-all",
                value.logic === "and"
        ...
        <Button
          ...
          size="sm"
          onClick={addFilter}
          className="text-[10px] font-bold text-cmm-purple hover:text-white"
        >
      ...
      {value.filters.map((filter, index) => {
        ...
        const valOpts =
          filter.field === "tag"
            ...
            : getValueOptions(filter.field);
        const isDate = isDateField(filter.field);

        return (
          <div
            key={index}
            className="flex flex-wrap items-center gap-2 rounded-xl border border-n-border bg-n-surface p-2.5"
          >
            {/* Field */}
            <select
              className="h-8 rounded-lg border border-n-border bg-black/30 px-2 text-xs font-semibold text-white outline-none"
              value={filter.field}
              onChange={(e) => {
            ...
            {operators.length > 1 && (
              <select
                ...
                onChange={(e) =>
                  updateFilter(index, { operator: e.target.value as SegmentFilterOperator })
                }
              >
                {operators.map((op) => (
            ...
            {valOpts ? (
              <select
                className="h-8 flex-1 min-w-[120px] rounded-lg border border-n-border bg-black/30 px-2 text-xs font-semibold text-white outline-none"
                value={filter.value}
              ...
              <input
                type="date"
                className="h-8 flex-1 min-w-[140px] rounded-lg border border-n-border bg-black/30 px-2 text-xs font-semibold text-white outline-none"
                value={filter.value}
                onChange={(e) => updateFilter(index, { value: e.target.value })}
              />
            ) : (
            ...
            <button
              ...
              className="h-8 w-8 flex items-center justify-center rounded-lg text-slate-500 hover:text-red-400 hover:bg-red-500/10 transition-colors"
            >
              ...
              <div className="w-full flex justify-center py-0.5">
                <span
                  className={cn(
                    ...
                    value.logic === "and"
                      ? "text-cmm-blue bg-cmm-blue/10"
                      : "text-cmm-orange bg-cmm-orange/10",
                  )}
                >
