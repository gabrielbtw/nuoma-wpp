...
const ALL_STEP_TYPES: FlowStepType[] = [
  "text",
  "audio",
  "image",
  "video",
      ...
      <div className="grid gap-2 md:grid-cols-2">
        ...
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              onChange({ ...step, conditionAction: (e.target.value || null) as ConditionAction })
            }
          >
            <option value="" className="bg-slate-900">
              Selecione...
            </option>
            {conditionActionOptions.map((o) => (
              <option key={o.value} value={o.value} className="bg-slate-900">
                {o.label}
              </option>
            ))}
      ...
      <div className="glass-card mb-2.5 overflow-hidden rounded-[1.4rem] border-n-border bg-n-surface p-0 shadow-sm transition-all hover:bg-n-surface-2">
        ...
        <div className="space-y-3 p-3.5">
          <div className="flex flex-col gap-2.5 xl:flex-row xl:items-center xl:justify-between">
            <StepTypeSelector
              value={step.type}
              onChange={(type) => onChange(normalizeStepForType(step, type))}
            ...
            <div className="space-y-3">
              {step.type === "wait" ? (
                ...
                <div className="space-y-1.5">
                  ...
                  />
                  {config.showVarHints !== false &&
                    (step.type === "text" || step.type === "link") && (
                      <p className="text-[9px] text-slate-600">
                        Variaveis:{" "}
                        {"{{nome}} {{primeiro_nome}} {{telefone}} {{email}} {{instagram}}"}
                      </p>
                    )}
                </div>
              )}
