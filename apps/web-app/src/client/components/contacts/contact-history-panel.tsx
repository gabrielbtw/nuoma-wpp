        ...
        <div className="flex flex-1 items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">
              O histórico fica recolhido por padrão e mostra valores anteriores e novos quando
              disponíveis.
            </p>
          </div>
        ...
        <CardContent>
          {query.isLoading ? (
            <div className="space-y-3">
              {Array.from({ length: 3 }).map((_, index) => (
                <div
                  key={index}
                  className="rounded-[1.25rem] border border-n-border bg-slate-950/45 p-4"
                >
          ...
          {!query.isLoading && (query.data ?? []).length === 0 ? (
            <div className="text-sm text-slate-400">
              Nenhuma alteração relevante registrada até agora.
            </div>
          ) : null}
          <div className="space-y-3">
            {(query.data ?? []).map((item) => (
              <div
                key={item.id}
                className="rounded-[1.25rem] border border-n-border bg-slate-950/45 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
              >
