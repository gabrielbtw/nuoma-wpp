...
function StatusRow({
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-3 rounded-lg border p-3 transition-fast",
        cfg.border,
        cfg.bg,
      )}
    >
...
function MetricBox({
  ...
  return (
    <div className="rounded-lg border border-n-border bg-n-surface p-3">
      ...
      <p
        className={cn(
          "mt-1 font-mono text-h3",
          status && status !== "unknown" ? cfg.text : "text-n-text",
        )}
      >
        {value}
      </p>
...
function formatIdentity(channel: ChannelHealthRecord) {
  ...
  return type.includes("instagram")
    ? id.startsWith("@")
      ? id
      : `@${id}`
    : formatChannelDisplayValue("whatsapp", id);
}
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-caption text-n-text-muted mt-0.5">
            Runtime, workers, banco, canais e eventos
          </p>
        </div>
        <button
          onClick={() => {
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            <StatusRow
              icon={ShieldCheck}
              label="Sistema"
              value={String(data.overallStatus ?? "unknown").toUpperCase()}
              status={overallLevel}
            />
            <StatusRow
              icon={Clock3}
              label="Scheduler"
              value={String(scheduler.status ?? "offline").toUpperCase()}
              status={schedulerLevel}
            />
            <StatusRow
              icon={Database}
              label="SQLite"
              value="ONLINE"
              status="ok"
              detail={dbName(dbPath)}
            />
            <StatusRow
              icon={MessageCircleMore}
              label="WhatsApp"
              value={String(worker.status ?? "offline").toUpperCase()}
              status={workerLevel}
            ...
            <StatusRow
              icon={Instagram}
              label="Instagram"
              value={String(igStatus).toUpperCase()}
              status={igLevel}
            ...
            <StatusRow
              icon={Activity}
              label="Memoria"
              value={`${worker.memoryMb ?? 0} MB`}
              status={Number(worker.memoryMb ?? 0) > 600 ? "warn" : "ok"}
            />
          </div>
          ...
          <div className="grid gap-2 md:grid-cols-4">
            <MetricBox
              label="Auth WA"
              value={String(worker.authStatus ?? "N/A")}
              status={worker.authStatus === "authenticated" ? "ok" : "warn"}
            />
            <MetricBox
              label="Falhas consecutivas"
              value={String(worker.consecutiveFailures ?? 0)}
              status={Number(worker.consecutiveFailures ?? 0) > 0 ? "warn" : "ok"}
            />
            <MetricBox
              label="Campanhas ativas"
              value={String(data.metrics?.activeCampaigns ?? 0)}
            />
            <MetricBox
              label="Conversas esperando"
              value={String(data.metrics?.waitingConversations ?? 0)}
              status={Number(data.metrics?.waitingConversations ?? 0) > 10 ? "warn" : undefined}
            />
          </div>
            ...
            <div className="rounded-xl border border-n-border bg-n-surface">
              <div className="flex items-center justify-between border-b border-n-border px-4 py-2.5">
                ...
                <span className="text-micro text-n-text-dim">
                  {channelList.length} configurados
                </span>
              </div>
              <div className="divide-y divide-n-border-subtle">
                {channelList.map((ch) => {
                  const isIg = String(ch.label ?? "")
                    .toLowerCase()
                    .includes("instagram");
                  const ChIcon = isIg ? Instagram : MessageCircleMore;
                  const chStatus = resolveStatus(
                    ch.account?.status || ch.worker?.status || ch.mode,
                  );
                    ...
                    <div key={ch.label} className="flex items-center gap-3 px-4 py-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={cn("signal-dot", cfg.dot)} />
                          <span className={cn("text-micro", cfg.text)}>
                            {ch.account?.status || ch.mode || "—"}
                          </span>
                        </div>
                      ...
                      <div className="flex gap-4 text-right">
                        <div>
                          ...
                          <p className="font-mono text-body font-semibold text-n-text">
                            {ch.mappedConversations ?? 0}
                          </p>
                        </div>
                        <div>
                          <p className="text-micro text-n-text-dim">Canais</p>
                          <p className="font-mono text-body font-semibold text-n-text">
                            {ch.mappedContactChannels ?? 0}
                          </p>
                        </div>
                      </div>
                    </div>
            ...
            <div className="rounded-xl border border-n-red/20 bg-n-red/5 p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-body text-n-text">
                    {worker.lastFailureSummary || "Sem detalhes"}
                  </p>
                  <div className="mt-2 flex gap-4 text-caption text-n-text-muted">
                    ...
                    <span>
                      ...
                      {worker.lastFailureAt
                        ? new Date(String(worker.lastFailureAt)).toLocaleString()
                        : "—"}
                    </span>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar max-h-[calc(100vh-16rem)]">
            <div className="p-3 space-y-0.5 font-mono text-caption">
              {(logsQuery.data?.events ?? []).map((event, i) => (
                <div
                  key={event.id || i}
                  className="flex gap-2 rounded px-2 py-1 hover:bg-n-surface-2 transition-fast"
                >
                  <span className="shrink-0 text-n-text-dim">
                  ...
                  <span
                    className={cn(
                      "shrink-0 text-micro uppercase w-10",
                      event.level === "error"
                        ? "text-n-red"
                        : event.level === "warn"
                          ? "text-n-amber"
