...
function ChannelIcon({ channel, size = 14 }: { channel: string; size?: number }) {
  if (channel === "instagram")
    return <Instagram style={{ width: size, height: size }} className="text-n-ig" />;
...
function ContactSidebar({
  ...
  const attachmentCandidatesQuery = useQuery({
    ...
    queryFn: () =>
      apiFetch<AttachmentCandidatesResponse>(
        `/contacts/${contactId}/attachment-candidates?limit=3`,
      ),
    enabled: Boolean(contactId),
  ...
  if (!contactId) {
    return (
      <div
        className={cn(
          inSheet ? "flex" : "hidden xl:flex",
          "flex-col items-center justify-center rounded-2xl border border-n-border/60 bg-n-surface p-6 text-n-text-dim",
        )}
      >
  ...
  return (
    <div
      className={cn(
        inSheet ? "flex" : "hidden xl:flex",
        "flex-col gap-3 overflow-y-auto custom-scrollbar pr-1",
      )}
    >
      ...
      <div className="rounded-2xl border border-n-border/60 bg-n-surface p-4 space-y-4">
        ...
        <div className="flex flex-col items-center text-center gap-2">
          <div
            className={cn(
              ...
              contact?.instagram
                ? "bg-gradient-to-br from-n-blue to-cmm-purple shadow-cmm-purple/15"
                : "bg-gradient-to-br from-n-blue to-blue-600 shadow-n-blue/15",
            )}
          >
          ...
          {editing ? (
            <Input
              className="h-8 text-center text-h4 bg-n-bg border-n-border rounded-lg"
              value={draft.name ?? ""}
            ...
            {channels.map((ch) => (
              <div
                ...
                className={cn(
                  ...
                  ch === "whatsapp"
                    ? "bg-n-wa/8 text-n-wa ring-n-wa/20"
                    : "bg-n-ig/8 text-n-ig ring-n-ig/20",
                )}
              >
                {ch === "whatsapp" ? (
        ...
        <div className="space-y-2 pt-3 border-t border-n-border/40">
          <div className="space-y-1">
            ...
            {editing ? (
              <Input
                className="h-8 text-caption bg-n-bg border-n-border font-mono"
                value={draft.phone ?? ""}
          ...
          <div className="space-y-1">
            ...
            {editing ? (
              <Input
                className="h-8 text-caption bg-n-bg border-n-border"
                value={draft.email ?? ""}
          ...
          <div className="space-y-1">
            ...
            {editing ? (
              <Textarea
                className="min-h-[48px] text-caption bg-n-bg border-n-border"
                value={draft.notes ?? ""}
        ...
        <div className="pt-2 border-t border-n-border/40 space-y-1.5">
          ...
          <div className="flex flex-wrap gap-1">
            {(editing ? draft.tags : contact?.tags)?.map((tag) => (
              <span
                key={tag}
                className="flex items-center gap-1 rounded-full bg-n-surface-2 px-2 py-0.5 text-micro text-n-text-muted ring-1 ring-white/[0.04]"
              >
                {tag}
                {editing && (
                  <button
                    onClick={() => removeTag(tag)}
                    className="text-n-red hover:text-red-300 ml-0.5"
                  >
            ...
            {(editing ? draft.tags : contact?.tags)?.length === 0 && (
              <span className="text-micro text-n-text-dim italic">Sem tags</span>
            )}
          </div>
          {editing && (
            <div className="flex gap-1">
              <Input
                className="h-7 flex-1 text-micro bg-n-bg border-n-border rounded-lg"
                value={tagInput}
              ...
              <button
                ...
                className="h-7 px-2 rounded-lg bg-n-surface-2 text-micro text-n-text-dim hover:text-n-text transition-fast ring-1 ring-white/[0.04]"
              >
                +
              </button>
            </div>
          ...
          {attachmentCandidates.length > 0 ? (
            <div className="space-y-1">
              {attachmentCandidates.map((item) => (
                <div
                  key={item.id}
                  className="flex items-center gap-2 rounded-lg bg-n-bg px-2.5 py-1.5 ring-1 ring-white/[0.04]"
                >
                  <AttachmentCandidateIcon type={item.contentType} />
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-caption text-n-text">
                      {formatAttachmentType(item.contentType)}
                    </p>
                    <p className="truncate font-mono text-micro text-n-text-dim">
                      {item.sha256.slice(0, 12)}
                    </p>
                  </div>
                  <span className="shrink-0 text-micro text-n-text-dim">
        ...
        <div className="pt-2 border-t border-n-border/40 space-y-2">
          {editing && (
            <div className="flex gap-1.5">
              ...
              <button
                ...
                className="h-8 w-8 rounded-lg bg-n-surface-2 text-n-text-dim flex items-center justify-center hover:bg-n-surface-2 ring-1 ring-white/[0.04] transition-fast"
              >
          ...
          <div className="flex gap-1.5">
            ...
            <button
              onClick={() => {
                if (confirm("Excluir este contato?")) deleteMutation.mutate();
              }}
...
export function InboxPage() {
  ...
  const [attachment, setAttachment] = useState<{ file: File; path: string; type: string } | null>(
    null,
  );
  ...
  const messagesQuery = useQuery({
    ...
    queryFn: () =>
      apiFetch<UnifiedMessage[]>(`/inbox/contact/${selectedEntry!.contactId}/messages`),
  ...
  const sendMutation = useMutation({
    ...
    onMutate: async ({ contactId, text, ch, mediaPath, contentType }) => {
      ...
      queryClient.setQueryData(
        ["contact-messages", selectedEntry?.contactId],
        (old: UnifiedMessage[] | undefined) => [
          ...(old ?? []),
          {
  ...
  const automationsQuery = useQuery({
    ...
    queryFn: () =>
      apiFetch<Array<{ id: string; name: string; enabled: boolean; category: string }>>(
        "/automations",
      ),
    staleTime: 30_000,
  });
  const enabledAutomations = useMemo(
    () => (automationsQuery.data ?? []).filter((a) => a.enabled),
            ...
            <div className="flex items-center gap-1.5">
              {[
                ...
                <button
                  ...
                  className={cn(
                    "h-7 w-7 rounded-md flex items-center justify-center transition-fast",
                    channel === opt.value
                      ? "bg-n-blue/10 text-n-blue"
                      : "text-n-text-dim hover:bg-n-surface-2",
                  )}
                >
            ...
            <div className="px-1 py-0.5">
              {entries.map((entry) => {
                ...
                return (
                  <button
                    key={entry.contactId}
                    onClick={() => setSelectedContactId(entry.contactId)}
                    className={cn(
                      "group relative flex w-full items-center gap-2.5 rounded-xl px-2.5 py-2 text-left transition-all duration-200",
                      isSelected
                        ? "bg-n-blue/10 text-n-text ring-1 ring-n-blue/15"
                        : "hover:bg-n-surface-2/60",
                    )}
                  >
                    <div
                      className={cn(
                        "flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-caption font-semibold transition-all duration-200",
                        isSelected
                          ? "bg-n-blue text-white shadow-sm shadow-n-blue/20"
                          : "bg-n-surface-2 text-n-text-dim ring-1 ring-white/[0.04]",
                      )}
                    >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center justify-between gap-2">
                        <p
                          className={cn(
                            "truncate text-h4",
                            isSelected ? "text-white" : "text-n-text",
                          )}
                        >
                          {entry.contactName ||
                            entry.contactPhone ||
                        ...
                        <span
                          className={cn(
                            "text-micro shrink-0",
                            isSelected ? "text-blue-200" : "text-n-text-dim",
                          )}
                        >
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        <p
                          className={cn(
                            "truncate text-caption flex-1",
                            isSelected ? "text-blue-100/70" : "text-n-text-muted",
                          )}
                        >
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  {selectedEntry?.channels.map((ch) => (
                    <span
                      key={ch}
                      className="flex items-center gap-1 text-micro uppercase text-n-text-dim"
                    >
                  ...
                  {selectedEntry && (
                    <Badge tone="default" className="text-micro px-1.5 py-0">
                      {selectedEntry.conversationCount} threads
                    </Badge>
                  )}
                </div>
              </div>
              ...
              >
                ...
                >
                  ...
                  {msg.contentType === "image" && msg.mediaPath && (
                    <img
                      src={`/uploads/media/${msg.mediaPath.split("/").pop()}`}
                      alt="imagem"
                      className="rounded-lg max-w-full max-h-60 mb-1.5"
                      loading="lazy"
                    />
                  )}
                  ...
                  {msg.contentType === "file" && msg.mediaPath && (
                    <a
                      href={`/uploads/media/${msg.mediaPath.split("/").pop()}`}
                      target="_blank"
                  ...
                  <div
                    className={cn(
                      "mt-1 flex items-center gap-1.5 text-micro opacity-50",
          ...
          <footer className="p-3 border-t border-n-border/40 bg-n-surface">
            <div className="flex items-end gap-2">
              ...
              <div className="flex flex-col gap-1">
                {selectedEntry?.channels.includes("whatsapp") && (
                  <button
                    ...
                    className={cn(
                      "h-8 w-8 rounded-lg flex items-center justify-center border transition-fast",
                      sendChannel === "whatsapp"
                        ? "border-n-wa/40 bg-n-wa/10"
                  ...
                  >
                    <MessageCircleMore
                      className={cn(
                        "h-3.5 w-3.5",
                        sendChannel === "whatsapp" ? "text-n-wa" : "text-n-text-dim",
                      )}
                    />
                  </button>
                )}
                {selectedEntry?.channels.includes("instagram") && (
                  ...
                  >
                    <Instagram
                      className={cn(
                        "h-3.5 w-3.5",
                        sendChannel === "instagram" ? "text-n-ig" : "text-n-text-dim",
                      )}
                    />
                  </button>
                )}
              ...
              <div className="flex flex-col gap-1">
                <label
                  className={cn(
                    "h-8 w-8 rounded-lg flex items-center justify-center border cursor-pointer transition-fast",
                    attachment
                      ? "border-n-blue/40 bg-n-blue/10 text-n-blue"
                      : "border-n-border bg-n-surface text-n-text-dim hover:bg-n-surface-2",
                  )}
                >
                    ...
                    <>
                      ...
                      <div className="absolute bottom-full left-0 mb-1 w-56 rounded-lg border border-n-border bg-n-surface shadow-panel z-50 max-h-48 overflow-y-auto custom-scrollbar">
                        <div className="px-3 py-1.5 border-b border-n-border">
                          <span className="text-micro text-n-text-dim font-medium">
                            Disparar automacao
                          </span>
                        </div>
              ...
              <div className="flex-1">
                {attachment && (
                  <div className="flex items-center gap-2 rounded-t-lg bg-n-surface-2 border border-b-0 border-n-border px-2 py-1.5">
                    ...
                    <span className="text-caption text-n-text-muted truncate flex-1">
                      {attachment.file.name}
                    </span>
                    <button
                      onClick={() => setAttachment(null)}
                      className="text-n-text-dim hover:text-n-red text-micro"
                    >
                      X
                    </button>
                  </div>
                ...
                <Textarea
                  placeholder={
                    uploading
                      ? "Enviando arquivo..."
                      : `Enviar via ${sendChannel === "whatsapp" ? "WhatsApp" : "Instagram"}...`
                  }
                  className={cn(
                    "min-h-[48px] max-h-[100px] resize-none bg-n-surface border-n-border px-3 py-2 text-body text-n-text focus:border-n-blue/40 transition-fast",
              ...
              <button
                disabled={
                  (!composer.trim() && !attachment) || !selectedEntry || sendMutation.isPending
                }
                onClick={doSend}
