...
import {
  type ContactProcedureStatus,
  contactProcedureLabelMap,
  contactStatusLabelMap,
  contactStatusTone,
  formatInstagramRelationship,
...
function channelSummary(contact: ContactRecord) {
  ...
  return [
    ...
    formatChannelDisplayValue("instagram", contact.instagram),
  ]
    .filter(Boolean)
    .join(" • ");
}
  ...
  return (
    <div className="divide-y divide-n-border-subtle">
      {Array.from({ length: 8 }).map((_, index) => (
        <div
          key={index}
          className="grid gap-3 px-4 py-3 lg:grid-cols-[minmax(0,2fr)_minmax(14rem,1.35fr)_auto] lg:items-center"
        >
...
export function ContactsPage() {
  ...
  const cpfError =
    draft.cpf.trim().length > 0 && !isValidCpf(draft.cpf)
      ? "CPF invalido. Confira o numero digitado antes de salvar."
      : null;
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="sm"
              className="h-8 rounded-lg text-micro uppercase text-n-text-muted hover:text-n-text"
              onClick={goBack}
            >
            ...
            <Link to="/contacts/lab/client-tab">
              <Button
                variant="ghost"
                size="sm"
                className="h-8 rounded-lg border border-n-border bg-n-surface text-micro uppercase text-n-text-muted hover:text-n-text transition-fast"
              >
                Ver 4 versoes
              </Button>
            </Link>
            ...
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild>
                <Button
                  ...
                  className="h-8 rounded-lg bg-n-blue px-4 text-micro uppercase text-white hover:bg-n-blue/90 transition-fast"
                >
                  ...
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <DialogTitle className="text-h2 text-n-text">
                        {editingId ? "Editar Perfil" : "Criar Registro"}
                      </DialogTitle>
                      <DialogDescription className="text-caption text-n-text-muted">
                        Preencha os dados principais do contato. O cadastro exige ao menos um canal
                        ativo.
                      </DialogDescription>
                    </div>
                    <div className="rounded-lg border border-n-border bg-n-surface-2 p-3 text-right">
                      ...
                      <p className="text-caption text-n-blue">
                        {draft.phone || draft.instagram || "Aguardando canal..."}
                      </p>
                    </div>
                  </div>

                  {saveContactMutation.error ? (
                    <ErrorPanel message={(saveContactMutation.error as Error).message} />
                  ) : null}
                    ...
                    <div className="space-y-4">
                      <div className="grid gap-3 md:grid-cols-2">
                        <div className="md:col-span-2 space-y-1">
                          ...
                          <Input
                            className="h-9 rounded-lg border-n-border bg-n-surface-2 px-3 text-body text-n-text focus:border-n-blue/40"
                            placeholder="Ex: Gabriel Braga"
                            value={draft.name}
                        ...
                        <div className="space-y-1">
                          ...
                          <Input
                            ...
                            value={draft.phone}
                            onChange={(e) =>
                              setDraft({ ...draft, phone: formatPhoneForInput(e.target.value) })
                            }
                          />
                        </div>
                        <div className="space-y-1">
                          ...
                          <Input
                            ...
                            value={draft.cpf}
                            onChange={(e) =>
                              setDraft({ ...draft, cpf: formatCpfInput(e.target.value) })
                            }
                          />
                        </div>
                      </div>
                    ...
                    <div className="space-y-4">
                      <div className="rounded-xl border border-n-border bg-n-surface-2 p-4 space-y-4">
                        <div className="space-y-1">
                          ...
                          <select
                            className="w-full h-9 rounded-lg border border-n-border bg-n-bg px-3 text-body text-n-text outline-none focus:border-n-blue/40"
                            value={draft.status}
                            onChange={(e) => setDraft({ ...draft, status: e.target.value })}
                          >
                            {Object.entries(contactStatusLabelMap).map(([v, l]) => (
                              <option key={v} value={v} className="bg-n-bg">
                                {l}
                              </option>
                            ))}
                          </select>
                        ...
                        <div className="space-y-1">
                          ...
                          <select
                            className="w-full h-9 rounded-lg border border-n-border bg-n-bg px-3 text-body text-n-text outline-none focus:border-n-blue/40"
                            value={draft.procedureStatus}
                            onChange={(e) =>
                              setDraft({
                                ...draft,
                                procedureStatus: e.target.value as ContactProcedureStatus,
                              })
                            }
                          >
                            <option value="unknown" className="bg-n-bg">
                              Nao Definido
                            </option>
                            <option value="yes" className="bg-n-bg">
                              Sim
                            </option>
                            <option value="no" className="bg-n-bg">
                              Nao
                            </option>
                          </select>
                        </div>
                      ...
                      <div className="space-y-1">
                        ...
                        <Textarea
                          className="min-h-[100px] rounded-xl border-n-border bg-n-surface-2 px-3 py-2 text-body text-n-text-muted focus:border-n-blue/40"
                          placeholder="Detalhes estrategicos..."
                          value={draft.notes || ""}
                  ...
                  <div className="flex items-center justify-between border-t border-n-border pt-6">
                    <p className="text-micro text-n-text-dim italic">
                      {hasPrimaryChannel
                        ...
                        : "Um canal (WPP/IG) e obrigatorio"}
                    </p>
                    <div className="flex gap-3">
                      <Button
                        variant="ghost"
                        className="h-9 rounded-lg px-4 text-label text-n-text-muted hover:bg-n-surface-2"
                        onClick={() => setDialogOpen(false)}
                      >
                        Cancelar
                      </Button>
                      <Button
                        className="h-9 rounded-lg bg-n-blue px-6 text-label text-white hover:bg-n-blue/90 transition-fast"
                        disabled={
                          saveContactMutation.isPending || !hasPrimaryChannel || Boolean(cpfError)
                        }
                        onClick={() => saveContactMutation.mutate(draft)}
                      >
      ...
      <div className="rounded-2xl border border-n-border/60 bg-n-surface p-4 space-y-4">
        <div className="grid gap-3 items-center lg:grid-cols-[1fr_auto]">
          <div className="grid gap-3 md:grid-cols-3">
            ...
            <select
              className="h-9 rounded-lg border border-n-border bg-n-surface-2 px-3 text-body text-n-text-muted outline-none transition-fast hover:bg-n-surface-2 focus:border-n-blue/40"
              value={statusFilter}
            ...
            >
              ...
              {(tagsQuery.data ?? []).map((item) => (
                <option key={item.id} value={item.name} className="bg-n-bg">
                  {item.name}
                </option>
              ))}
            </select>
              ...
              <tbody className="divide-y divide-n-border/30">
                {contactsQuery.isLoading ? (
                  ...
                  (contactsQuery.data?.items ?? []).map((contact) => (
                    <tr
                      key={contact.id}
                      className="group transition-all duration-200 hover:bg-n-surface-2/50"
                    >
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div
                            className={cn(
                              "flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-body font-semibold transition-all duration-200 ring-1",
                              contact.instagram
                                ? "bg-n-ig/8 text-n-ig ring-n-ig/15"
                                : "bg-n-wa/8 text-n-wa ring-n-wa/15",
                            )}
                          >
                            {(
                          ...
                          <div className="min-w-0 space-y-0.5">
                            <h4 className="truncate text-h4 text-n-text leading-tight">
                              {contact.name || "Sem Nome"}
                            </h4>
                            <p className="truncate text-micro uppercase text-n-text-dim">
                              {channelSummary(contact)}
                            </p>
                          </div>
                        </div>
                      </td>
                          ...
                          <div className="flex flex-wrap gap-1 opacity-70 group-hover:opacity-100 transition-fast">
                            {contact.tags.slice(0, 2).map((item) => (
                              <TagPill
                                key={`${contact.id}-${item}`}
                                name={item}
                                color={tagColorMap.get(item.trim().toLowerCase())}
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <div className="flex flex-col">
                            <span className="text-micro text-n-text-dim">
                              {contactProcedureLabelMap[contact.procedureStatus]} Proc.
                            </span>
                            {contact.instagram && (
                              <span className="text-micro text-n-blue mt-0.5">
                                {formatInstagramRelationship(contact)}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-fast">
                          <Button
                            variant="ghost"
                            className="h-8 w-8 rounded-lg bg-n-surface-2 hover:bg-n-blue/10 hover:text-n-blue transition-fast"
                            onClick={() => openForEdit(contact)}
                          >
                            <Pencil className="h-3.5 w-3.5" />
                          </Button>
                          <Link to={`/contacts/${contact.id}`}>
                            <Button
                              className="h-8 rounded-lg border border-n-border bg-n-surface-2 px-3 text-micro uppercase text-n-text-muted hover:bg-n-surface-2 hover:text-n-text transition-fast"
                              variant="secondary"
                            >
                              Dossie
                            </Button>
                ...
                {!contactsQuery.isLoading && (contactsQuery.data?.items ?? []).length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-4 py-16 text-center">
                      ...
                      <p className="text-caption text-n-text-muted mt-1">
                        Ajuste sua busca ou cadastre um novo paciente para operar.
                      </p>
                    </td>
                  </tr>
                )}
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 pt-3 border-t border-n-border/40">
          <div className="space-y-0.5">
            ...
            <div className="flex items-center gap-2">
              <Badge
                tone="default"
                className="rounded-full px-2 py-0.5 text-micro bg-n-surface-2 border-n-border"
              >
                Pagina {currentPage} de {totalPages}
              </Badge>
              <span className="text-caption text-n-text-muted italic">
                Visualizando {pageStart}-{pageEnd} registros
              </span>
            </div>
          </div>
          <div className="flex items-center gap-2">
