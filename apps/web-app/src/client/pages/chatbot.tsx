...
function emptyChatbot() {
  return {
    ...
    channelScope: "any",
    description: "",
    fallbackAction: "silence_and_flag",
    fallbackTag: "chatbot_nao_entendeu",
    rules: [emptyRule()],
  };
}
...
export function ChatbotPage() {
  ...
  function selectChatbot(bot: ChatbotRecord) {
    ...
    setDraft({
      name: bot.name,
      enabled: bot.enabled,
      channelScope: bot.channelScope,
      description: bot.description,
      fallbackAction: bot.fallbackAction,
      fallbackTag: bot.fallbackTag,
      rules: bot.rules.length > 0 ? bot.rules : [emptyRule()],
    });
  }

  // Simulate match test
  const testResult = testInput.trim()
    ? draft.rules.find((r) => {
    ...
    <div className="flex h-full flex-col gap-4 overflow-hidden animate-fade-in">
      <PageHeader
        ...
        actions={
          <Button
            onClick={startCreate}
            className="bg-cmm-purple text-white transition-all duration-200 hover:brightness-110"
          >
      ...
      <div className="grid flex-1 gap-4 overflow-hidden xl:grid-cols-[320px_1fr_300px]">
        ...
        <div className="flex flex-col gap-3 overflow-hidden">
          <div className="flex-1 overflow-y-auto space-y-1.5 custom-scrollbar">
            {chatbots.map((bot) => (
              <button
                key={bot.id}
                onClick={() => selectChatbot(bot)}
                className={cn(
                  "w-full flex items-center gap-3 rounded-xl p-3 text-left transition-all duration-200",
                  selectedId === bot.id
                    ? "bg-cmm-purple/10 border border-cmm-purple/30"
                    : "hover:bg-n-surface-2 border border-transparent",
                )}
              >
                <Bot
                  className={cn(
                    "h-5 w-5 shrink-0",
                    bot.enabled ? "text-cmm-purple" : "text-n-text-dim",
                  )}
                />
                <div className="min-w-0 flex-1">
                  <p className="text-body font-semibold text-n-text truncate">{bot.name}</p>
                  <p className="text-micro text-n-text-dim">
                    {bot.rules.length} regras | {bot.channelScope}
                  </p>
                </div>
                {bot.enabled ? (
                  <Power className="h-3.5 w-3.5 text-n-wa" />
                ) : (
                  <PowerOff className="h-3.5 w-3.5 text-n-text-dim" />
                )}
              </button>
            ))}
            {chatbots.length === 0 && (
              <EmptyState
                ...
                description="Crie seu primeiro chatbot para responder automaticamente."
                actionLabel="Criar chatbot"
        ...
        <div className="flex flex-col gap-4 overflow-y-auto custom-scrollbar">
          {isEditing ? (
            <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-2">
                      {[
                        ...
                        <button
                          key={opt.value}
                          onClick={() => setDraft({ ...draft, channelScope: opt.value })}
                          className={cn(
                            "flex-1 flex items-center justify-center gap-1.5 h-9 rounded-lg border text-xs font-bold transition-all",
                            draft.channelScope === opt.value
                              ? "border-cmm-purple/30 bg-cmm-purple/10 text-cmm-purple"
                              : "border-n-border/40 bg-n-surface text-n-text-dim",
                          )}
                        >
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-body font-semibold text-n-text">
                    Regras ({draft.rules.length})
                  </h3>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => setDraft({ ...draft, rules: [...draft.rules, emptyRule()] })}
                    className="text-cmm-purple"
                  >
                ...
                {draft.rules.map((rule, idx) => (
                  <div
                    key={idx}
                    className="rounded-xl border border-n-border/40 bg-n-surface p-3 space-y-2.5"
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="text-n-text-dim hover:text-n-red"
                        >
                    ...
                    <div className="grid gap-2 md:grid-cols-[120px_1fr]">
                      <select
                        className="h-9 rounded-lg border border-n-border/40 bg-n-surface-2 px-2 text-xs font-semibold text-n-text"
                        value={rule.matchType}
                        onChange={(e) => {
                          const rules = [...draft.rules];
                          rules[idx] = { ...rule, matchType: e.target.value };
                          setDraft({ ...draft, rules });
                        }}
                      >
                        {matchTypeOptions.map((o) => (
                          <option key={o.value} value={o.value} className="bg-n-bg">
                            {o.label}
                          </option>
                        ))}
                      </select>
                      <Input
                        ...
                        onChange={(e) => {
                          const rules = [...draft.rules];
                          rules[idx] = { ...rule, keywordPattern: e.target.value };
                          setDraft({ ...draft, rules });
                        }}
                      />
                    </div>
                    <Textarea
                      ...
                      onChange={(e) => {
                        const rules = [...draft.rules];
                        rules[idx] = { ...rule, responseBody: e.target.value };
                        setDraft({ ...draft, rules });
                      }}
                    />
                    <div className="flex items-center gap-3">
                      <label className="flex items-center gap-1.5 text-micro text-n-text-dim">
                        <input
                          type="checkbox"
                          checked={rule.flagForHuman}
                          onChange={(e) => {
                            const rules = [...draft.rules];
                            rules[idx] = { ...rule, flagForHuman: e.target.checked };
                            setDraft({ ...draft, rules });
                          }}
                          className="rounded"
                        />
                      ...
                      <Input
                        ...
                        onChange={(e) => {
                          const rules = [...draft.rules];
                          rules[idx] = { ...rule, applyTag: e.target.value || null };
                          setDraft({ ...draft, rules });
                        }}
                      />
                    </div>
                    <div className="flex items-center gap-3">
                      <Input
                        ...
                        placeholder="Filtro DDD"
                        value={rule.phoneDddFilter ?? ""}
                        onChange={(e) => {
                          const rules = [...draft.rules];
                          rules[idx] = { ...rule, phoneDddFilter: e.target.value || null };
                          setDraft({ ...draft, rules });
                        }}
                      />
                      <select
                        className="h-7 rounded-lg border border-n-border/40 bg-n-surface-2 px-2 text-xs text-n-text"
                        value={rule.triggerAutomationId ?? ""}
                        onChange={(e) => {
                          const rules = [...draft.rules];
                          rules[idx] = { ...rule, triggerAutomationId: e.target.value || null };
                          setDraft({ ...draft, rules });
                        }}
                      >
              ...
              <div className="rounded-xl border border-n-amber/20 bg-n-amber/5 p-3 space-y-2">
                ...
                <p className="text-micro text-n-text-muted">
                  Quando nenhuma regra faz match: silencio + flag com tag "{draft.fallbackTag}"
                </p>
              </div>
              ...
              <div className="flex gap-3">
                <Button
                  className="flex-1 bg-cmm-purple text-white transition-all duration-200 hover:brightness-110"
                  disabled={!draft.name.trim() || saveMutation.isPending}
                  onClick={() =>
                    saveMutation.mutate({ id: selectedId ?? undefined, payload: draft })
                  }
                >
            ...
            <EmptyState
              icon={Bot}
              title="Selecione ou crie um chatbot"
              description="Escolha um chatbot da lista ou crie um novo para configurar regras de resposta automatica."
            />
          )}
        </div>
        ...
        <div className="hidden xl:flex flex-col gap-4">
              ...
              <div className="space-y-2">
                {testResult ? (
                  <div className="rounded-lg bg-n-wa/10 border border-n-wa/20 p-3">
                    <p className="text-micro font-semibold text-n-wa uppercase tracking-wider">
                      Match encontrado
                    </p>
                    <p className="mt-1 text-caption text-n-text-muted">
                      Keyword: {testResult.keywordPattern}
                    </p>
                    <div className="mt-2 rounded-lg bg-n-surface-2 p-2">
                      <p className="text-body text-n-text">
                        {testResult.responseBody || "(sem resposta configurada)"}
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="rounded-lg bg-n-amber/10 border border-n-amber/20 p-3">
                    <p className="text-micro font-semibold text-n-amber uppercase tracking-wider">
                      Sem match
                    </p>
                    <p className="mt-1 text-caption text-n-text-muted">
                      Fallback: silencio + flag "{draft.fallbackTag}"
                    </p>
                  </div>
                )}
              </div>
          ...
          <div className="flex-1 rounded-2xl border border-n-border/40 bg-n-surface p-4 space-y-3 overflow-y-auto">
            ...
            <div className="space-y-2">
                  ...
                  {testResult ? (
                    <div className="flex justify-start">
                      <div className="rounded-2xl rounded-bl-md bg-cmm-purple/20 px-3 py-2 max-w-[80%]">
                        <p className="text-caption text-n-text">
              ...
              {!testInput.trim() && (
                <p className="text-micro text-n-text-dim text-center py-8">
                  Digite uma mensagem acima para testar
                </p>
              )}
            </div>
          </div>
