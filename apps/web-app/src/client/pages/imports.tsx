  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <PageHeader
        eyebrow="Operacoes"
        title="Importacoes"
        description="Instagram import, CSV matching, enriquecimento e limpeza."
      />
            ...
            {batch ? (
              <>
                <div className="flex items-center gap-2">
                  ...
                  <span className="ml-auto text-micro text-n-text-dim">
                    {new Date(batch.created_at).toLocaleString()}
                  </span>
                </div>
                <div className="grid gap-2 md:grid-cols-2">
                  {[
                    ["Threads", n(agg.processedThreads)],
                    ["Seguidores", n(agg.processedFollowers)],
                    ["Seguindo", n(agg.processedFollowing)],
                    ["Telefones", n(agg.phonesDiscovered)],
                    ["CSV matches", n(agg.whatsappCsvMatches)],
                    ["Nomes CSV", n(agg.whatsappCsvNamesApplied)],
                    ["Nomes tel", n(agg.namesFromPhones)],
                    ["Sem JSON", n(agg.skippedNoSupportedData)],
                  ].map(([label, value]) => (
                    <div
                      key={String(label)}
                      className="flex items-center justify-between rounded-lg bg-n-surface-2 px-3 py-2"
                    >
                      <span className="text-caption text-n-text-muted">{label}</span>
                      <span className="font-mono text-body font-semibold text-n-text">
                        {String(value)}
                      </span>
                    </div>
                  ))}
                ...
                {batch.meta.csvPath && (
                  <div className="rounded-lg bg-n-surface-2 px-3 py-2 text-caption text-n-text-muted">
                    CSV: {batch.meta.csvPath}
                  </div>
                )}
                ...
                {n(msgEnrich.updatedContacts) > 0 && (
                  <div className="rounded-lg border border-n-ig/20 bg-n-ig/5 px-3 py-2 text-caption text-n-text">
                    Mensagens: {n(msgEnrich.updatedContacts)} contatos, {n(msgEnrich.namesApplied)}{" "}
                    nomes, {n(msgEnrich.cpfsApplied)} CPFs
                  </div>
                )}
              ...
              <p className="py-8 text-center text-caption text-n-text-dim">
            ...
            {fileRuns.length > 0 ? (
              fileRuns.slice(0, 24).map((event) => {
                ...
                return (
                  <div
                    key={event.id}
                    className="px-4 py-2.5 hover:bg-n-surface-2/50 transition-fast"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-body font-medium text-n-text truncate">
                        {event.meta.sourcePath?.split("/").pop() ?? event.message}
                      </span>
                      <Badge
                        tone={n(summary.skippedNoSupportedData) > 0 ? "warning" : "success"}
