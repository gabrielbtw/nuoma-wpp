...
type UploadedCsvResponse = {
  ...
  headers: string[];
  preview: Array<
    Record<string, string> & { _normalizedPhone?: string | null; _phoneLooksValid?: boolean }
  >;
  totalRows: number;
};
...
function campaignCompletion(campaign: CampaignRecord) {
  ...
  return Math.max(
    0,
    Math.min(100, Math.round((campaign.processedRecipients / campaign.totalRecipients) * 100)),
  );
}
...
export function CampaignsPage() {
  ...
  const recipientsQuery = useQuery({
    ...
    queryFn: () =>
      apiFetch<CampaignRecipientRecord[]>(`/campaigns/${selectedCampaign?.id}/recipients`),
  ...
  useEffect(() => {
    ...
    if (
      firstCampaign &&
      (!selectedCampaignId ||
        !filteredCampaigns.some((campaign) => campaign.id === selectedCampaignId))
    ) {
  ...
  useEffect(() => {
    ...
    setMapping((current) => ({
      phone: csvUpload.headers.includes(current.phone)
        ? current.phone
        : guessHeader(csvUpload.headers, [
            "phone",
            "telefone",
            "celular",
            "numero",
      ...
      name: csvUpload.headers.includes(current.name)
        ? current.name
        : guessHeader(csvUpload.headers, ["name", "nome"]),
      instagram: csvUpload.headers.includes(current.instagram)
        ? current.instagram
        : guessHeader(csvUpload.headers, ["instagram", "ig", "insta"]),
      tags: csvUpload.headers.includes(current.tags)
        ? current.tags
        : guessHeader(csvUpload.headers, ["tag", "tags", "etiqueta", "etiquetas"]),
    }));
  }, [csvUpload]);
  ...
  const filteredRecipients = useMemo(() => {
    ...
    return recipientFilter === "all"
      ? items
      : items.filter((recipient) => recipient.status === recipientFilter);
  ...
  const importRecipientsMutation = useMutation({
    mutationFn: ({
      ...
      campaignId: string;
      payload: { uploadId: string; mapping: typeof mapping };
    }) =>
  ...
  const canActivateCampaign = Boolean(
    selectedCampaign &&
    activationIssues.length === 0 &&
    !["active", "completed", "cancelled"].includes(selectedCampaign.status),
  );
  const canPauseCampaign = selectedCampaign?.status === "active";
  const canCancelCampaign = Boolean(
    selectedCampaign && !["cancelled", "completed"].includes(selectedCampaign.status),
  );
  const importableRecipients = importableCount(csvValidation?.summary);
  const canImportRecipients = Boolean(
    selectedCampaign?.id &&
    csvUpload?.uploadId &&
    hasCsvMapping &&
    !csvValidating &&
    importableRecipients > 0,
  );
  ...
  return (
    <div className="space-y-5 animate-fade-in">
            ...
            <DialogContent className="h-[min(94dvh,980px)] w-[min(1280px,96vw)] overflow-hidden p-0">
              <div className="flex h-full flex-col">
                <div className="border-b border-n-border/40 px-6 py-5">
                  <DialogTitle className="text-h2 text-n-text">
                    Nova campanha omnichannel
                  </DialogTitle>
                  <DialogDescription className="mt-1 text-caption text-n-text-muted">
                    Monte o fluxo, salve o rascunho e depois importe ou vincule os destinatários no
                    painel principal.
                  </DialogDescription>
                </div>
                <div className="flex-1 overflow-y-auto px-6 py-6">
                  {createMutation.error ? (
                    <ErrorPanel message={(createMutation.error as Error).message} />
                  ) : null}
                  <CampaignBuilder
                    value={draft}
                    onChange={(next) => setDraft(normalizeCampaignDraft(next))}
                  />
                </div>
                <div className="flex items-center justify-between border-t border-n-border/40 px-6 py-4">
                  <div className="text-caption text-n-text-muted">
                    Ao salvar, o rascunho entra na lista principal.
                  </div>
                  <Button
      ...
      {flashMessage ? (
        <div className="rounded-xl border border-n-wa/20 bg-n-wa/[0.06] px-4 py-3 text-caption text-n-wa">
          {flashMessage}
        </div>
      ) : null}
      {campaignsQuery.error ? (
        <ErrorPanel message={(campaignsQuery.error as Error).message} />
      ) : null}
        ...
        <div className="space-y-3">
          <div className="rounded-2xl border border-n-border/60 bg-n-surface p-5 space-y-4">
            <div>
              ...
              <p className="mt-0.5 text-caption text-n-text-dim">
            ...
            <div className="space-y-2.5">
              ...
              >
                <option value="all" className="bg-n-bg">
                  Todos os status
                </option>
              ...
              {(filters.query || filters.status !== "all") && (
                <button
                  className="text-caption text-n-text-dim hover:text-n-text transition-fast"
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isSelected
                        ? "bg-n-blue/[0.06] border-l-2 border-l-n-blue"
                        : "hover:bg-n-surface-2/50 border-l-2 border-l-transparent",
                    )}
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      >
                        ...
                        <p className="mt-1 line-clamp-1 text-caption text-n-text-dim">
                          {campaign.description || "Sem resumo informado."}
                        </p>
                      </button>
        ...
        <div className="space-y-4">
          {selectedCampaign ? (
                ...
                <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1.9fr)_270px]">
                  <div className="min-w-0">
                    <div className="space-y-2">
                      <div className="flex items-start gap-3">
                        <h2 className="min-w-0 flex-1 text-h1 text-n-text">
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                          ...
                          {[
                            {
                              ...
                              active: selectedCampaign.eligibleChannels.includes("whatsapp"),
                              tone: "text-n-wa border-n-wa/25 bg-n-wa/10",
                            },
                            {
                              key: "instagram",
                              icon: Instagram,
                              active: selectedCampaign.eligibleChannels.includes("instagram"),
                              tone: "text-cmm-orange border-cmm-orange/25 bg-cmm-orange/10",
                            },
                          ].map((channel) => {
                            ...
                            return (
                              <div
                                ...
                                className={cn(
                                  ...
                                  channel.active
                                    ? channel.tone
                                    : "border-n-border bg-n-surface text-slate-600",
                                )}
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {[
                            ...
                            <div
                              key={item.label}
                              className="rounded-full border border-n-border bg-n-surface px-3 py-1.5"
                            >
                              <span className="text-[9px] font-semibold uppercase tracking-[0.18em] text-n-text-dim">
                                {item.label}
                              </span>
                              <span className="ml-2 text-sm font-bold tracking-tight text-n-text">
                                {item.value}
                              </span>
                            </div>
                          ))}
                  ...
                  <div className="rounded-[1.4rem] border border-n-border bg-black/20 p-3.5">
                    <div className="space-y-1.5">
                      <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-n-text-dim">
                        Janela operacional
                      </p>
                      <p className="text-[15px] font-bold tracking-tight text-n-text">
                        {selectedCampaign.sendWindowStart}h - {selectedCampaign.sendWindowEnd}h
                      </p>
                      <p className="text-xs text-n-text-dim">
                        {selectedCampaign.rateLimitCount} envios a cada{" "}
                        {selectedCampaign.rateLimitWindowMinutes} minutos
                      </p>
                    </div>
                    <div className="mt-3 grid gap-2 sm:grid-cols-2">
                      <Button
                        ...
                        size="sm"
                        className={cn(
                          "h-10 rounded-[1.15rem] px-3.5 text-[13px] font-bold shadow-lg",
                          canActivateCampaign
                            ? "bg-n-blue text-n-text hover:bg-n-blue/90"
                            : "bg-white/5 text-n-text-dim",
                        )}
                      ...
                      <Button
                        ...
                        size="sm"
                        className={cn(
                          "h-10 rounded-[1.15rem] px-3.5 text-[13px] font-bold",
                          canPauseCampaign
                            ? "bg-white/10 text-n-text hover:bg-white/20"
                            : "bg-white/5 text-n-text-dim",
                        )}
                      ...
                      <Button
                        ...
                        className={cn(
                          "h-10 rounded-[1.15rem] px-3.5 text-[13px] font-bold",
                          canCancelCampaign
                            ? "bg-cmm-orange/12 text-cmm-orange hover:bg-cmm-orange/20"
                            : "bg-white/5 text-n-text-dim",
                        )}
                ...
                <div className="border-b border-n-border px-4 bg-n-surface">
                  <ChromeTabs
                    ...
                    items={[
                      {
                        value: "campaign",
                        label: "Workflow do Builder",
                        badge: editorCampaign?.steps.length ?? selectedCampaign.steps.length,
                      },
                  ...
                  <div className="animate-in space-y-9 p-8 fade-in duration-500">
                    <div className="space-y-8">
                      {editorCampaign ? (
                        <CampaignBuilder
                          ...
                          onChange={(next) => setEditorCampaign(normalizeCampaignDraft(next))}
                        />
                      ) : null}
                      <div className="flex justify-end border-t border-n-border pt-5">
                        <Button
                          ...
                          onClick={() =>
                            editorCampaign && saveExistingMutation.mutate(editorCampaign)
                          }
                        >
                    ...
                    <div className="border-t border-n-border pt-12">
                      <div className="flex flex-wrap items-center justify-between gap-6">
                        <div className="space-y-1">
                          <h3 className="text-2xl font-bold text-n-text tracking-tight">
                            Importação de Base
                          </h3>
                          <p className="text-sm font-medium text-n-text-muted">
                            Arraste seu arquivo CSV para processar e vincular contatos
                            automaticamente.
                          </p>
                        </div>
                      ...
                      {csvUpload ? (
                        <div className="mt-8 space-y-8 animate-in slide-in-from-top-4 duration-500">
                          <div className="grid gap-4 xl:grid-cols-4">
                            {[
                              ...
                              <div key={field.key} className="space-y-1.5">
                                <label className="ml-1 text-[10px] font-bold uppercase tracking-wider text-n-text-dim">
                                  {field.label}
                                </label>
                                <select
                                ...
                                >
                                  <option value="" className="bg-n-bg">
                                    {field.label} (opcional)
                                  </option>
                                  {csvUpload.headers.map((header) => (
                                    <option key={header} value={header} className="bg-n-bg">
                                      {header}
                                    </option>
                                  ))}
                                </select>
                          ...
                          <div className="flex flex-wrap gap-3">
                            <Badge
                              tone="default"
                              className="bg-white/5 border-n-border text-[10px] uppercase font-bold tracking-wider px-3 py-1"
                            >
                              {csvUpload.totalRows} linhas no arquivo
                            </Badge>
                            <Badge
                              tone="default"
                              className="bg-n-blue/10 border-n-blue/20 text-n-blue text-[10px] uppercase font-bold tracking-wider px-3 py-1"
                            >
                              {selectedCampaign.eligibleChannels.join(" + ")}
                            </Badge>
                            {csvValidating ? (
                            ...
                            <div className="space-y-6">
                              <div className="grid gap-4 md:grid-cols-3 xl:grid-cols-6">
                                {[
                                  {
                                    label: "Existentes",
                                    value: csvValidation.summary.existing,
                                    tone: "info" as const,
                                  },
                                  {
                                    label: "Vinculáveis",
                                    value: csvValidation.summary.eligible,
                                    tone: "warning" as const,
                                  },
                                  {
                                    label: "Novos",
                                    value: csvValidation.summary.new_contact,
                                    tone: "success" as const,
                                  },
                                  {
                                    label: "Revisar",
                                    value: csvValidation.summary.needs_review,
                                    tone: "warning" as const,
                                  },
                                  {
                                    label: "Parcial",
                                    value: csvValidation.summary.insufficient_link,
                                    tone: "warning" as const,
                                  },
                                  {
                                    label: "Inválidos",
                                    value: csvValidation.summary.invalid,
                                    tone: "danger" as const,
                                  },
                                ].map((item) => (
                                  <div
                                    key={item.label}
                                    className="rounded-3xl border border-n-border bg-n-surface p-5"
                                  >
                                    <div className="text-[9px] font-semibold uppercase tracking-[0.2em] text-n-text-dim">
                                      {item.label}
                                    </div>
                                    <div className="mt-2 text-2xl font-bold text-n-text tracking-tighter">
                                      {item.value}
                                    </div>
                                  </div>
                                ))}
                                  ...
                                  <table className="w-full text-left text-sm">
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                      {csvValidation.preview.map((row, index) => (
                                        <tr
                                          key={index}
                                          className="group hover:bg-n-surface transition-colors"
                                        >
                                          {csvValidation.headers.map((header) => (
                                            <td
                                              key={header}
                                              className="px-6 py-4 text-xs font-medium text-n-text"
                                            >
                                              {previewCellValue(row[header]) || "-"}
                                            </td>
                                          ))}
                                          <td className="px-6 py-4 text-xs font-bold text-n-blue">
                                            {row._resolvedChannel
                                              ? formatChannelDisplayValue(
                                                  row._resolvedChannel,
                                                  row._resolvedTargetDisplay,
                                                )
                                              : "-"}
                                          </td>
                                          <td className="px-6 py-4 text-right">
                                            <Badge
                                              tone={previewTone(row._exists)}
                                              className="text-[9px] uppercase font-semibold tracking-wider"
                                            >
                                              {previewLabel(row._exists)}
                                            </Badge>
                                          </td>
                                        </tr>
                                      ))}
                              ...
                              <div className="flex items-center justify-between gap-6 pt-4">
                                <p className="text-xs font-medium text-n-text-dim italic">
                                  {importableRecipients} destinatários prontos para importação
                                  automática.
                                </p>
                                <Button
                                  variant="secondary"
                                  ...
                                  disabled={
                                    !canImportRecipients || importRecipientsMutation.isPending
                                  }
                                  onClick={() =>
                                    importRecipientsMutation.mutate({
                                      campaignId: selectedCampaign.id,
                                      payload: { uploadId: csvUpload.uploadId, mapping },
                                    })
                                  }
                                >
                        ...
                        <div className="mt-8 rounded-3xl border border-dashed border-n-border bg-n-surface px-6 py-12 text-center">
                          ...
                          <p className="mt-4 text-xs font-bold text-n-text-dim uppercase tracking-wider italic">
                            Nenhum CSV carregado. Arraste ou selecione um arquivo para começar.
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="border-t border-n-border pt-12">
                      <div className="space-y-1 mb-6">
                        ...
                        <p className="text-sm font-medium text-n-text-muted">
                          Digite numeros de WhatsApp ou usernames do Instagram, um por linha.
                        </p>
                      </div>
                      <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
                        <div className="space-y-3">
                          ...
                          <p className="text-[10px] text-n-text-dim">
                            ...
                            {manualChannel === "whatsapp" ? "numeros" : "usernames"} digitados
                          </p>
                        </div>
                  ...
                  <div className="animate-in fade-in duration-500">
                        ...
                        <button
                          ...
                          className={cn(
                            ...
                            recipientFilter === item.key
                              ? "border-n-blue/30 bg-n-blue/10 text-n-text"
                              : "border-n-border bg-n-surface text-n-text-muted hover:bg-n-surface-2",
                          )}
                        >
                          ...
                          <span
                            className={cn(
                              "text-[10px] font-semibold opacity-40 px-2 py-0.5 rounded-full bg-white/10",
                              recipientFilter === item.key && "bg-n-blue text-n-text opacity-100",
                            )}
                          >
                            {item.count}
                          </span>
                    ...
                    <div className="grid gap-px bg-white/5 xl:grid-cols-[1fr_400px]">
                        ...
                        <div className="max-h-[520px] divide-y divide-white/5 overflow-y-auto custom-scrollbar">
                              ...
                              >
                                  ...
                                  <div
                                    className={cn(
                                      ...
                                      recipient.channel === "instagram"
                                        ? "bg-cmm-orange/10 text-cmm-orange"
                                        : "bg-n-wa/10 text-n-wa",
                                    )}
                                  >
                                    {recipient.name?.charAt(0) ||
                                      recipient.target_display_value?.charAt(0) ||
                                      "?"}
                                  </div>
                                  <div className="min-w-0">
                                    <h4 className="truncate text-[15px] font-bold text-n-text tracking-tight">
                                      {recipient.name || "Sem identificação"}
                                    </h4>
                                    <p className="mt-0.5 truncate text-[11px] font-bold text-n-text-dim uppercase tracking-wider">
                                      {formatRecipientTarget(recipient) || "Alvo não determinado"}
                                    </p>
                                  </div>
                                </div>
                                <div className="flex shrink-0 flex-col items-end gap-2">
                                  <Badge
                                    ...
                                    className="rounded-full px-2.5 py-0.5 text-[8px] font-semibold uppercase tracking-wider"
                                  >
                                  ...
                                  <span className="text-[9px] font-semibold text-slate-600 uppercase tracking-wider">
                                    STEP {Math.max(Number(recipient.step_index), 0) + 1}
                                  </span>
                                </div>
                                {isSelected && (
                                  <div className="absolute left-0 top-1/2 -translate-y-1/2 h-10 w-1 bg-n-blue rounded-r-full shadow-[0_0_20px_rgba(59,130,246,0.5)]" />
                                )}
                              </button>
                            );
                          })}
                          ...
                          {filteredRecipients.length === 0 && (
                            <div className="p-20 text-center">
                              <p className="text-xs font-bold text-n-text-dim uppercase tracking-wider italic opacity-40">
                        ...
                        {filteredRecipients.length > 0 ? (
                          <div className="flex items-center justify-between border-t border-n-border px-4 py-2.5 text-[10px] font-semibold uppercase tracking-wider text-n-text-dim">
                            <span>
                              ...
                              {" - "}
                              {Math.min(
                                recipientPage * RECIPIENTS_PER_PAGE,
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setRecipientPage((current) =>
                                    Math.min(recipientTotalPages, current + 1),
                                  )
                                }
                      ...
                      <div className="border-l border-n-border bg-slate-950/60 p-5">
                        <h5 className="mb-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-n-text-dim">
                          Detalhes do contato
                        </h5>
                        {selectedRecipient ? (
                          <div className="animate-in space-y-6 fade-in slide-in-from-right-4 duration-500">
                            <div>
                              <h3 className="text-lg font-bold text-n-text tracking-tight">
                                {selectedRecipient.name || "Sem Nome"}
                              </h3>
                              <p className="mt-1 text-sm font-medium text-n-text-muted">
                                {formatRecipientTarget(selectedRecipient)}
                              </p>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2.5">
                              <div className="rounded-2xl border border-n-border bg-white/5 p-3.5">
                                ...
                                <Badge
                                  tone={recipientTone(selectedRecipient.status)}
                                  className="mt-2 text-[9px] uppercase font-semibold"
                                >
                                  {recipientStatusLabels[selectedRecipient.status]}
                                </Badge>
                              </div>
                              <div className="rounded-2xl border border-n-border bg-white/5 p-3.5">
                                <div className="text-[9px] font-semibold text-slate-600 uppercase tracking-wider">
                                ...
                                <div className="mt-2 text-lg font-bold text-n-text tracking-tighter">
                                  #{Math.max(Number(selectedRecipient.step_index), 0) + 1}
                                </div>
                              </div>
                            </div>
                              ...
                              {[
                                {
                                  ...
                                  value: formatCampaignDateTime(selectedRecipient.last_attempt_at),
                                },
                                {
                                  label: "Próximo Agendamento",
                                  value: formatCampaignDateTime(selectedRecipient.next_run_at),
                                },
                                {
                                  label: "Canal Ativo",
                                  value:
                                    selectedRecipient.channel === "instagram"
                                      ? "Instagram"
                                      : "WhatsApp",
                                },
                              ].map((item) => (
                                <div
                                  key={item.label}
                                  className="flex items-center justify-between border-b border-n-border py-2"
                                >
                                  <span className="text-[10px] font-bold text-n-text-dim uppercase tracking-wider">
                                    {item.label}
                                  </span>
                                  <span className="text-xs font-bold text-n-text">
                                    {item.value}
                                  </span>
                                </div>
                              ))}
                            ...
                            {selectedRecipient.last_error && (
                              <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-3.5">
                                <div className="text-[9px] font-semibold text-red-400 uppercase tracking-wider">
                                  Último Log de Erro
                                </div>
                                <p className="mt-2 text-[11px] font-medium text-red-200 leading-relaxed">
                                  {selectedRecipient.last_error}
                                </p>
                              </div>
                            )}
                          ...
                          <div className="h-full flex flex-col items-center justify-center text-center opacity-30">
                            ...
                            <p className="mt-6 text-[10px] font-bold uppercase tracking-wider">
            ...
            <div className="flex flex-col items-center justify-center p-20 text-center rounded-2xl border-n-border bg-n-surface">
              ...
              <p className="mt-2 text-sm font-medium text-n-text-muted max-w-sm">
                Crie uma nova ou selecione ao lado para gerenciar fluxos e destinatários.
              </p>
            </div>
          )}
        </div>
      ...
      <Dialog
        open={Boolean(viewerCampaignId)}
        onOpenChange={(open) => !open && setViewerCampaignId(null)}
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-[#0c0c0e] border-n-border">
          ...
          <DialogDescription className="sr-only">
            Fluxo visual da campanha com estatisticas por etapa
          </DialogDescription>
          {viewerCampaign && (
            <WorkflowViewer
              campaignId={viewerCampaign.id}
              steps={
                viewerCampaign.steps as unknown as import("@/lib/campaign-utils").CampaignStepDraft[]
              }
