...
const versions = [
  {
    ...
    description:
      "Foco em acolhimento premium, leitura rápida e decisão de próximo passo em um bloco só.",
  },
  {
    name: "Versão 02",
    title: "Signal Board",
    description:
      "Mais analítica, com ênfase em sinais de relacionamento, intensidade e timing da conversa.",
  },
  {
    name: "Versão 03",
    title: "Narrative Ledger",
    description:
      "Organiza o cliente como uma mini história operacional, ótima para handoff e contexto.",
  },
  {
    name: "Versão 04",
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {tags.map((tag) => (
        <Badge
          key={tag}
          className="border-white/10 bg-white/[0.04] px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-slate-200"
        >
          {tag}
        </Badge>
      ))}
    ...
    <section className="rounded-[2rem] border border-cyan-400/15 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.16),transparent_38%),linear-gradient(160deg,rgba(15,23,42,0.96),rgba(2,6,23,0.92))] p-7 shadow-[0_30px_80px_-40px_rgba(34,211,238,0.35)]">
      <div className="flex flex-wrap items-start justify-between gap-6">
        ...
        <div className="rounded-[1.5rem] border border-cyan-300/20 bg-cyan-300/10 px-4 py-3">
          ...
          <p className="mt-2 max-w-xs text-sm font-medium leading-relaxed text-cyan-50">
            {mockClient.nextAction}
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <MiniMetric
          label="Status"
          value={mockClient.status}
          icon={Sparkles}
          accent="text-cyan-300"
        />
        <MiniMetric
          label="Procedimento"
          value={mockClient.procedure}
          icon={ShieldCheck}
          accent="text-emerald-300"
        />
        <MiniMetric
          label="Último toque"
          value={mockClient.lastTouch}
          icon={CalendarClock}
  ...
  return (
    <section className="rounded-[2rem] border border-lime-300/15 bg-[radial-gradient(circle_at_top_right,rgba(163,230,53,0.14),transparent_34%),linear-gradient(180deg,rgba(10,14,10,0.95),rgba(8,20,16,0.94))] p-7 shadow-[0_30px_80px_-42px_rgba(163,230,53,0.28)]">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.24em] text-lime-200/70">
            Signal board
          </p>
          <h2 className="mt-1 font-display text-2xl font-semibold tracking-tight text-white">
            {mockClient.name}
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MiniMetric
          ...
          value={`${mockClient.incomingMessages}`}
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
        ...
        <div className="rounded-[1.5rem] border border-lime-200/15 bg-lime-200/[0.06] p-5">
          <p className="text-[10px] font-black uppercase tracking-[0.24em] text-lime-100/70">
            Recomendação
          </p>
  ...
  return (
    <section className="rounded-[2rem] border border-fuchsia-300/15 bg-[radial-gradient(circle_at_top_left,rgba(244,114,182,0.16),transparent_36%),linear-gradient(180deg,rgba(24,7,18,0.95),rgba(10,10,18,0.95))] p-7 shadow-[0_30px_80px_-44px_rgba(244,114,182,0.24)]">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.24em] text-fuchsia-100/70">
            Narrative ledger
          </p>
          <h2 className="mt-1 font-display text-2xl font-semibold tracking-tight text-white">
            {mockClient.name}
    ...
    <div className="space-y-8 pb-16">
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {versions.map((version, index) => (
          <div
            key={version.name}
            className="rounded-[2.2rem] border border-white/8 bg-white/[0.02] p-4"
          >
