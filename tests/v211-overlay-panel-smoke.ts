...
async function main() {
  ...
  if (sendJobsDelta !== 0) {
    throw new Error(
      `overlay panel smoke changed active send jobs: before=${sendJobsBefore} after=${sendJobsAfter}`,
    );
  }
  ...
  return page.evaluate(
    ({ rootId, panelTestId, canaryPhone }) => {
      ...
      const hasContactActions =
        ...
        contactActions.some(
          (action) => action.text.includes("Sincronizar conversa") && !action.disabled,
        ) &&
        contactActions.some(
          (action) => action.text.includes("Copiar telefone") && !action.disabled,
        );
...
function assertPanel(panel: Awaited<ReturnType<typeof readPanelState>>, label: string) {
  if (!panel.panelVisible || panel.sections < 4) {
    throw new Error(
      `${label} overlay panel did not render expected sections: ${JSON.stringify(panel)}`,
    );
  }
  if (
    label === "fixture" &&
    (!panel.hasFixtureSummary || !panel.hasFixtureAutomations || !panel.hasFixtureNotes)
  ) {
