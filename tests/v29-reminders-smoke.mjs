...
const appScreenshotPath = process.env.APP_SCREENSHOT_PATH ?? "data/v29-reminders-m20-app.png";
const wppScreenshotPath = process.env.WPP_SCREENSHOT_PATH ?? "data/v29-reminders-m20-wpp.png";
...
const smokePhone = "5531999999625";
const smokeTitle = "V2.9.25 Reminders Smoke";
const smokeMarker = "V2.9.25 Reminders Smoke";
...
async function main() {
  ...
  const fixture = seedReminderFixture();
  const sendJobsBefore = countSendJobsForPhone(smokePhone);
  const title = `M20 lembrete funcional ${Date.now()}`;
  ...
  try {
    ...
    await page.getByTestId("inbox-reminders-tab").click();
    await page.getByTestId("inbox-reminder-form").waitFor({ state: "visible", timeout: 10_000 });
    await page.getByTestId("inbox-reminder-title").fill(title);
    await page.getByTestId("inbox-reminder-due-at").fill(tomorrowAtNine());
    await page.getByTestId("inbox-create-reminder").click();

    const reminderList = page.getByTestId("inbox-reminders-list");
    await reminderList.getByText(title).waitFor({ state: "visible", timeout: 10_000 });
    const rowsBeforeComplete = await page.getByTestId("inbox-reminder-row").count();
    if (rowsBeforeComplete < 1) {
      throw new Error("reminders tab did not render any reminder rows after create");
    }
    ...
    if (blocking.length > 0) {
      throw new Error(
        `inbox reminders has blocking a11y violations: ${blocking
          .map((violation) => `${violation.id}:${violation.impact}`)
          .join(", ")}`,
      );
    }

    const reminderBeforeComplete = readLatestReminder(title);
    if (reminderBeforeComplete?.status !== "open") {
      throw new Error(`created reminder is not open: ${JSON.stringify(reminderBeforeComplete)}`);
    }

    await page.getByTestId("inbox-complete-reminder").first().click();
    await page.waitForTimeout(750);

    const reminderAfterComplete = readReminderById(reminderBeforeComplete.id);
    if (reminderAfterComplete?.status !== "done" || !reminderAfterComplete.completedAt) {
      throw new Error(`reminder was not completed: ${JSON.stringify(reminderAfterComplete)}`);
    }
    ...
    if (sendJobsDelta !== 0) {
      throw new Error(`reminders smoke created ${sendJobsDelta} send job(s)`);
    }

    const wppMode = await captureWhatsAppPrint(wppScreenshotPath);
    console.log(
      `v29-reminders|conversation=${fixture.conversationId}|contact=${fixture.contactId}|rows=${rowsBeforeComplete}|created=${reminderBeforeComplete.id}|completed=${reminderAfterComplete.status}|sendJobsDelta=${sendJobsDelta}|blocking=${blocking.length}|app=${appScreenshotPath}|wpp=${wppScreenshotPath}|wppMode=${wppMode}`,
    );
  } finally {
    await browser.close();
  }
...
function seedReminderFixture() {
  ...
  try {
    ...
    const contact = db
      ...
      .get(smokePhone);
    if (!contact?.id) {
      throw new Error("reminders smoke contact was not created");
    }
    ...
    db.prepare(
      `
        ...
        VALUES (
          ...
          'smoke lembrete pequeno com data e texto', 0, 0, NULL,
    ...
    const conversation = db
      ...
      .get(smokePhone);
    if (!conversation?.id) {
      throw new Error("reminders smoke conversation was not created");
    }

    db.prepare("DELETE FROM messages WHERE user_id = 1 AND conversation_id = ?").run(
      conversation.id,
    );
    db.prepare("DELETE FROM reminders WHERE user_id = 1 AND conversation_id = ?").run(
      conversation.id,
    );
...
function readLatestReminder(title) {
  ...
  try {
    const row = db
      ...
      .get(title);
    return row
      ? { id: Number(row.id), status: String(row.status), completedAt: row.completedAt }
      : null;
  } finally {
    db.close();
  }
