...
export function getDashboardSummary() {
  ...
  const counts = {
    contacts: Number(
      (
        db.prepare("SELECT COUNT(*) AS count FROM contacts WHERE deleted_at IS NULL").get() as {
          count: number;
        }
      ).count,
    ),
    tags: Number(
      (db.prepare("SELECT COUNT(*) AS count FROM tags").get() as { count: number }).count,
    ),
    ...
    unreadConversations: Number(
      (
        db.prepare("SELECT COUNT(*) AS count FROM conversations WHERE unread_count > 0").get() as {
          count: number;
        }
      ).count,
    ),
    ...
    campaignsRunning: Number(
      (
        db
          .prepare(
            "SELECT COUNT(*) AS count FROM campaigns WHERE status IN ('ready', 'active', 'paused')",
          )
          .get() as { count: number }
      ).count,
    ),
  ...
  const failedJobsCount = Number(
    (
      db
        .prepare(
          "SELECT COUNT(*) AS count FROM jobs WHERE status = 'failed' AND datetime(updated_at) >= datetime('now', '-24 hours')",
        )
        .get() as { count: number }
    ).count,
  );
  ...
  const failedCampaignRecipients = Number(
    (
      db
        .prepare("SELECT COUNT(*) AS count FROM campaign_recipients WHERE status = 'failed'")
        .get() as { count: number }
    ).count,
  );
