...
export function listSystemEventsByProcess(processName: string, limit = 200, offset = 0) {
  ...
  return db
    .prepare(
      "SELECT * FROM system_logs WHERE process_name = ? ORDER BY datetime(created_at) DESC LIMIT ? OFFSET ?",
    )
...
export function getSettings() {
  const db = getDb();
  const rows = db.prepare("SELECT * FROM app_settings ORDER BY key ASC").all() as Array<{
    key: string;
    value_json: string;
    updated_at: string;
  }>;
